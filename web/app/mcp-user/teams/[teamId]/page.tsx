...
import { Users, Key, BookOpen, MessageSquare, Trash2, Plus, Copy, Check, ArrowLeft } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-3 flex items-center gap-3">
          <Link href="/mcp-user/portal/teams" className="text-gray-400 hover:text-gray-700 transition-colors flex-shrink-0">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div className="min-w-0">
            <h1 className="text-sm font-semibold text-gray-900 truncate">{teamName || '…'}</h1>
            <p className="text-xs text-gray-500 capitalize hidden sm:block">Your role: {role}</p>
          </div>
        </div>

        {/* Scrollable tab bar */}
        <div className="flex overflow-x-auto border-t border-gray-100 scrollbar-none max-w-4xl mx-auto px-4 md:px-6">
          {TABS.map(t => (
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 md:px-6 md:py-6 space-y-4">
        {newToken && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => copyToken(newToken)} className="flex-shrink-0 p-2 rounded-lg border hover:bg-gray-50">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {skills.map(s => (
                        <tr key={s.slug} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-gray-500 text-xs truncate max-w-[200px]">{s.summary || s.name}</td>
                          {isAdmin && (
                            <td className="px-4 py-3 text-right">
